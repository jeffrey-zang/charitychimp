...
export default function Browse() {
  ...
  charities.reverse();
  const [searchedCharities, setSearchedCharities] = React.useState<
    Charity[] | null
  >(charities);
  const [searchString, setSearchString] = React.useState<string>("");

  const handleSubmit = (
    e: React.SyntheticEvent | React.KeyboardEvent<HTMLInputElement>
  ) => {
    e.preventDefault();
    let searched = search(searchString);
    setSearchedCharities(
      searchString ? (searched?.length ? searched : null) : charities
    );
  ...
  const search: (search: string) => Charity[] = (search: string) => {
    ...
    for (let charity of charities) {
      if (
        ...
        charity.tagline.toLowerCase().includes(search)
      ) {
        found.push(charity);
        continue;
      }
            ...
            .map((value, index) => {
              ...
              return (
                <button className="post" key={value.name}>
                  <h2>
                    <BsCheckCircleFill
                      style={{ display: value.verified ? "inline" : "none" }}
                    />
                    {value.name}
                  </h2>
                  <p>{value.tagline}</p>
                  <a
                    href={value.website}
                    style={{ display: value.website ? "block" : "none" }}
                  >
                  ...
                  <div className="progress">
                    <div
                      style={{
                        width: `${(value.current / value.goal) * 100}%`,
                      }}
                    ></div>
                  </div>
                  <div className="progress-labels">
                    <span>
                      {Math.round((value.current / value.goal) * 100)}%
                    </span>
                  ...
                  <p
                    className="poster"
                    style={{
                      display: value.poster.length > 0 ? "flex" : "none",
                    }}
                  >
                    <img
                      src={value.poster.length > 0 ? value.poster[0] : ""}
                    ></img>
